...
/* things that get used by vulture later */
...
int vulture_typecount[NUM_TILETYPES];
...
/* parse the configuration file and prepare the data arrays needed by the rest of vulture 
* This function is the only one that should be called from outside */
void vulture_parse_tileconf(FILE *fp, gametiles **gt_ptr)
{
	...
	vulture_typecount[TT_MISC] = MISCTILECOUNT;
	tilenames[TT_MISC] = miscnames;
	vulture_typecount[TT_CURSOR] = V_CURSOR_MAX;
	tilenames[TT_CURSOR] = cursornames;

	vulture_typecount[TT_WALL] = 0;
	vulture_typecount[TT_FLOOR] = 0;
	vulture_typecount[TT_EDGE] = 0;

	for (i = 0; i < NUM_TILETYPES; i++)
		tmp_gametiles[i].resize(vulture_typecount[i]);
	...
	* there are, so offsets into the final tile array can be computed */
	...
	for (i = 1; i < NUM_TILETYPES; i++)
		typeoffset[i] = typeoffset[i-1] + vulture_typecount[i-1];
	...
	*gt_ptr = new gametiles[GAMETILECOUNT];
	...
	for (i = 0; i < NUM_TILETYPES; i++)
	{
		for (j = 0; j < vulture_typecount[i]; j++)
		{
			...
			if (!tmp_gametiles[i][j].filename.empty() &&
				...
				game_tiles[tilenum].filename = V_BASE_DIRECTORY + tmp_gametiles[i][j].filename;
		...
		* forward references which won't work until gametiles is fully initialized */
		for (i = 0; i < NUM_TILETYPES; i++)
		{
			for (j = 0; j < vulture_typecount[i]; j++)
			{
				...
				tile = vulture_load_tile(loadnum);
				if (!tile) {
					...
					vulture_write_log(V_LOG_NOTE, __FILE__, __LINE__, "Tile config: %s\n", messagebuf);
				}
				else
				{
...
int vulture_get_tile_index(int type, char * name, int allow_expand)
{
	...
	index = -1;
	for (i = 0; i < vulture_typecount[type]; i++)
		if (tilenames[type][i] == name)
			index = i;

	if (index == -1) {
		if ((type == TT_WALL || type == TT_FLOOR || type == TT_EDGE) && allow_expand) {
			index = vulture_typecount[type];

			vulture_typecount[type]++;
			tilenames[type].resize(vulture_typecount[type]);
			tmp_gametiles[type].resize(vulture_typecount[type]);
...
{
	int tilenum = vulture_get_tile_index(tiletype, tilename, 1);
...
void vulture_redirect_tile(int tiletype, char *tilename, int redir_tiletype, char *redir_tilename)
{
	...
	dst = vulture_get_tile_index(redir_tiletype, redir_tilename, 1);
	if (dst == -1)
		return;
	...
	{
		src = vulture_get_tile_index(tiletype, tilename, 1);
		if (src == -1)
			return;
								...
								char * half_w, char * half_n, char * half_s, char * half_e)
{
	...
	tmp_wallstyles[style_id][0] = vulture_get_tile_index(TT_WALL, full_w, 0);
	tmp_wallstyles[style_id][1] = vulture_get_tile_index(TT_WALL, full_n, 0);
	tmp_wallstyles[style_id][2] = vulture_get_tile_index(TT_WALL, full_s, 0);
	tmp_wallstyles[style_id][3] = vulture_get_tile_index(TT_WALL, full_e, 0);
	tmp_wallstyles[style_id][4] = vulture_get_tile_index(TT_WALL, half_w, 0);
	tmp_wallstyles[style_id][5] = vulture_get_tile_index(TT_WALL, half_n, 0);
	tmp_wallstyles[style_id][6] = vulture_get_tile_index(TT_WALL, half_s, 0);
	tmp_wallstyles[style_id][7] = vulture_get_tile_index(TT_WALL, half_e, 0);
								...
								char *edge09, char *edge10, char *edge11, char *edge12)
{
	...
	tmp_flooredges[style_id][0] = vulture_get_tile_index(TT_EDGE, edge01, 0);
	tmp_flooredges[style_id][1] = vulture_get_tile_index(TT_EDGE, edge02, 0);
	tmp_flooredges[style_id][2] = vulture_get_tile_index(TT_EDGE, edge03, 0);
	tmp_flooredges[style_id][3] = vulture_get_tile_index(TT_EDGE, edge04, 0);
	tmp_flooredges[style_id][4] = vulture_get_tile_index(TT_EDGE, edge05, 0);
	tmp_flooredges[style_id][5] = vulture_get_tile_index(TT_EDGE, edge06, 0);
	tmp_flooredges[style_id][6] = vulture_get_tile_index(TT_EDGE, edge07, 0);
	tmp_flooredges[style_id][7] = vulture_get_tile_index(TT_EDGE, edge08, 0);
	tmp_flooredges[style_id][8] = vulture_get_tile_index(TT_EDGE, edge09, 0);
	tmp_flooredges[style_id][9] = vulture_get_tile_index(TT_EDGE, edge10, 0);
	tmp_flooredges[style_id][10] = vulture_get_tile_index(TT_EDGE, edge11, 0);
	tmp_flooredges[style_id][11] = vulture_get_tile_index(TT_EDGE, edge12, 0);
...
static void init_objnames()
{
	...
	vulture_typecount[TT_OBJECT] = OBJTILECOUNT;
	...
	vulture_typecount[TT_OBJICON] = vulture_typecount[TT_OBJECT];
...
static void init_monnames()
{
	...
	vulture_typecount[TT_MONSTER] = MONTILECOUNT;
	...
	vulture_typecount[TT_STATUE] = vulture_typecount[TT_MONSTER];
	tilenames[TT_STATUE] = tilenames[TT_MONSTER];

	vulture_typecount[TT_FIGURINE] = vulture_typecount[TT_MONSTER];
...
static void init_explnames()
{
	...
	vulture_typecount[TT_EXPL] = EXPL_MAX * 9;
